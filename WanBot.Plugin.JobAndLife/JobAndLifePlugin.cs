    ...
    public class JobAndLifePlugin : WanBotPlugin
    {
        ...
        public async Task<bool> DoWork(ISender sender, bool overwork)
        {
            ...
            if (usr.Energy < 0)
            {
                ...
                usr.Money -= costMoney;
                await sender.ReplyAsync($"完犊子了，累到脑溢血还不算工伤，亏了 {costMoney:N} 元");
            }
            ...
            else
            {
                ...
                await sender.ReplyAsync($"今天也是辛苦的一天，赚了 {(BigInteger)salary:N} 元");
            }
